...
// SessionAwareUserAuth is retained as a descriptive alias for creation
// routes. rc.25 authenticates dashboard requests with a short-lived Bearer
// access token; legacy Gin session cookies are no longer accepted. Browser
// media requests must be issued through the authenticated API client rather
// than relying on a cookie-only <img> or <video> request.
