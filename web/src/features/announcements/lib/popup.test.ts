...
describe('announcement popup policy', () => {
  test('keeps legacy announcements disabled and filters future announcements', () => {
    ...
    expect(pending.map((item) => item.id)).toEqual([3])
  })
  ...
  test('editing announcement content creates a new popup version', () => {
    ...
    expect(getAnnouncementVersionKey(original)).not.toBe(
    ...
    ).toEqual(['Updated service notice'])
  })
  ...
  test('excludes setup, OAuth callback, and error routes', () => {
    expect(shouldShowAnnouncementPopupOnPath('/')).toBe(true)
    expect(shouldShowAnnouncementPopupOnPath('/sign-in')).toBe(true)
    expect(shouldShowAnnouncementPopupOnPath('/setup')).toBe(false)
    expect(shouldShowAnnouncementPopupOnPath('/oauth/github')).toBe(false)
    expect(shouldShowAnnouncementPopupOnPath('/500')).toBe(false)
    expect(
