...
export function Rankings() {
  ...
  const errorStatus = (
    rankingsQuery.error as { response?: { status?: number } } | null
  )?.response?.status
  ...
  if (rankingsQuery.isLoading) {
    ...
    let errorMessage = t('Unable to load rankings data')
    if (errorStatus === 403) {
      errorMessage = t('Usage rankings are disabled by the administrator.')
    } else if (rankingsQuery.error instanceof Error) {
      errorMessage = rankingsQuery.error.message
    }
    rankingsContent = <RankingsError message={errorMessage} />
  } else {
    rankingsContent = (
      <>
        <ModelsSection
